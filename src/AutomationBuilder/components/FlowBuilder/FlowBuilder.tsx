...
const AddStepNode = ({ data }: any) => {
  ...
  return (
    <div className="w-[280px] flex flex-col items-center justify-center relative">
      <div
        className={`w-10 h-10 rounded-full bg-slate-50 border-2 border-dashed flex items-center justify-center transition-colors cursor-pointer group ${
...
const NodeCard = ({ id, icon: Icon, title, subtitle, colorClass, selected, isLastBranchNode, isDragging, isRoot, isTargetable, hideSourceHandle }: any) => {
  ...
  return (
    <div className="relative w-[280px]">
      <div
        className={`w-full bg-white rounded-lg shadow-sm border-2 transition-all duration-200 group relative z-10 ${
          selected
            ? 'border-primary ring-2 ring-primary/20'
            : isTargetable
              ? 'border-green-500 ring-2 ring-green-500/20 cursor-crosshair animate-pulse'
      ...
      >
        ...
        {!isTargetable && (
          <Handle
            ...
            className={`!w-3 !h-3 !bg-slate-300 !border-2 !border-white transition-colors hover:!bg-primary ${isRoot ? '!opacity-0 !border-0' : ''}`}
          />
        )}
        {!isTargetable && !hideSourceHandle && (
          <Handle type="source" position={Position.Bottom} className="!w-3 !h-3 !bg-slate-300 !border-2 !border-white transition-colors hover:!bg-primary" />
        )}
      </div>
      ...
      {isLastBranchNode && !isTargetable && !hideSourceHandle && (
        <>
          ...
          <Handle
            ...
            id="right-source"
            className={`!w-auto !h-auto !bg-transparent !border-none !rounded-none !left-[330px] !right-auto !top-1/2 !-translate-y-1/2 !transform-none !mt-3 !flex !flex-col !items-center !justify-center !gap-2 group/adder !opacity-100 !pointer-events-auto`}
          >
            <div
              className={`w-10 h-10 rounded-full bg-white border-2 border-dashed flex items-center justify-center transition-colors shadow-sm ${
...
const TriggerNode = ({ id, data, selected }: any) => {
  ...
  return (
    <div
      className={`w-[280px] bg-white rounded-lg shadow-sm border-2 transition-all duration-200 group ${
        selected
          ? 'border-primary ring-2 ring-primary/20'
          : data.isTargetable
            ? 'border-green-500 ring-2 ring-green-500/20 cursor-crosshair animate-pulse'
...
const EndNode = ({ data, selected }: any) => (
  <div className="w-[280px] flex flex-col items-center">
    <div
      className={`w-6 h-6 rounded-full bg-slate-200 border-2 flex items-center justify-center mb-1 ${
...
const LoopBackNode = ({ id, data, selected }: any) => {
  ...
  return (
    <div
      className={`w-[280px] bg-white rounded-lg shadow-sm border-2 transition-all duration-200 group relative ${
        selected
          ? 'border-primary ring-2 ring-primary/20'
          : data.isConnecting
    ...
    (event: React.DragEvent) => {
      ...
      if (foundItem) {
        newNode.data = {
          ...
          icon: foundItem.icon,
          subtitle:
            foundItem.label === 'Birthday'
              ? "Triggers on contact's birthday"
              : foundItem.label === 'Notes'
                ? 'Note Added'
                : type === 'trigger'
                  ? 'When this happens...'
                  : type === 'condition'
                  ...
                  <TabsContent key={section.value} value={section.value} className="m-0 h-full p-2 space-y-2 mt-0">
                    {available.map((item, idx) => (
                      <div
                        key={`avail-${idx}`}
                        draggable
                        className={`flex items-center gap-2 p-1.5 rounded-md border border-transparent hover:border-slate-200 hover:bg-slate-50 cursor-grab active:cursor-grabbing transition-all group ${
                      ...
                      <>
                        ...
                        {comingSoon.map((item, idx) => (
                          <div
                            key={`soon-${idx}`}
                            draggable={false}
                            className={`flex items-center gap-2 p-1.5 rounded-md border border-transparent opacity-75 cursor-not-allowed group select-none ${
                              isSidebarCollapsed ? 'justify-center' : ''
                            }`}
                          >
