...
const DEFAULT_CONDITION: IfElseCondition = { field: 'tag', operator: 'contains', value: '' };
...
const normalizeBranch = (
  ...
  const conditions = Array.isArray(branch.conditions) && branch.conditions.length > 0
    ? branch.conditions
    : [{ ...DEFAULT_CONDITION }];

  const label = branch.label || (idx === 0 ? (legacyIfLabel || 'If') : `Else If ${idx}`);
...
const buildSubtitle = (branches: IfElseBranch[]) => {
  ...
  if (branches.length === 1 && totalConditions === 1 && first) {
    ...
    return `If ${fieldLabel} ${operatorLabel}`;
  }
  return `${branches.length} Condition Branch${branches.length > 1 ? 'es' : ''}`;
};
...
export const IfElseConfig = ({ data, onChange }: { data: any, onChange: (d: any) => void, edges: any[], node: Node }) => {
  ...
  const elseLabel = data.elseLabel || data.falseLabel || 'Else';
  ...
  const updateBranches = (nextBranches: IfElseBranch[], nextElseLabel = elseLabel) => {
    ...
    const legacyOperator = normalized[0]?.logicalOperator || 'and';
  ...
  const addCondition = (branchIndex: number) => {
    const next = branches.map((branch, idx) => {
      ...
      return { ...branch, conditions: [...branch.conditions, { ...DEFAULT_CONDITION }] };
    });
    updateBranches(next);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between px-0.5">
        <div>
          <Label className="text-[12px] font-bold text-slate-700 dark:text-slate-200 leading-none">Conditional Branches</Label>
          <p className="text-[10px] text-slate-400 mt-1 uppercase tracking-tight">Evaluate in order, fall back to else</p>
        </div>
        <Button variant="ghost" size="sm" onClick={addBranch} className="h-7 text-xs font-bold text-primary hover:text-primary hover:bg-primary/5 rounded-lg border border-primary/20">
          <PlusIcon className="w-3 h-3 mr-1" /> Add Else If
        </Button>
      </div>
          ...
          <div key={branch.id} className="p-4 bg-slate-50/50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 rounded-xl space-y-4">
            <div className="flex items-center justify-between">
              <div className="text-[11px] font-bold text-slate-600 dark:text-slate-300 uppercase tracking-widest">
                {branchIndex === 0 ? 'If' : `Else If ${branchIndex}`}
              </div>
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder={branchIndex === 0 ? 'If' : 'Else If'}
              />
            </div>
            ...
            {branch.conditions.length > 1 && (
              <div className="flex items-center gap-2 p-1 bg-slate-100 dark:bg-slate-900/50 rounded-lg w-fit border border-slate-200 dark:border-slate-800">
                <button
                  type="button"
                  onClick={() => updateBranch(branchIndex, { logicalOperator: 'and' })}
                  className={cn(
                    "px-3 py-1 rounded-md text-[10px] font-bold transition-all",
                    branch.logicalOperator === 'and' ? "bg-white dark:bg-slate-800 text-primary shadow-sm" : "text-slate-500 hover:text-slate-700"
                  )}
                >
                  AND
                </button>
                <button
                  type="button"
                  onClick={() => updateBranch(branchIndex, { logicalOperator: 'or' })}
                  className={cn(
                    "px-3 py-1 rounded-md text-[10px] font-bold transition-all",
                    branch.logicalOperator === 'or' ? "bg-white dark:bg-slate-800 text-primary shadow-sm" : "text-slate-500 hover:text-slate-700"
                  )}
                >
                  OR
                </button>
            ...
            <div className="space-y-3">
              {branch.conditions.map((c, i) => (
                <div key={`${branch.id}-${i}`} className="p-4 bg-white/80 dark:bg-slate-900/70 border border-slate-100 dark:border-slate-800 rounded-xl space-y-4 relative">
                  {branch.conditions.length > 1 && (
                    <button
                      type="button"
