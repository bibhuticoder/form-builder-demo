    ...
    <div className="space-y-4 pb-6">
      <div className="space-y-4">
        ...
        {block.type === EmailBlockType.HEADING && (
          <>
            <div>
              ...
              <input type="text" value={(block as HeadingBlock).content} onChange={(e) => handleUpdate("content", e.target.value)} className={inputClass} />
            </div>
            <div>
        ...
        {block.type === EmailBlockType.TEXT && (
          <div>
            <label className={labelClass}>Content</label>
            <textarea value={(block as TextBlock).content} onChange={(e) => handleUpdate("content", e.target.value)} className={textareaClass} rows={4} />
          </div>
        )}
        ...
        {block.type === EmailBlockType.IMAGE && (
          <>
            <div>
              ...
              <input type="text" value={(block as ImageBlock).src} onChange={(e) => handleUpdate("src", e.target.value)} className={selectClass} placeholder="https://..." />
            </div>
            <div>
              <label className={labelClass}>Alt Text</label>
              <input type="text" value={(block as ImageBlock).alt || ""} onChange={(e) => handleUpdate("alt", e.target.value)} className={selectClass} />
            </div>
            <div>
              <label className={labelClass}>Link URL</label>
              <input type="text" value={(block as ImageBlock).linkUrl || ""} onChange={(e) => handleUpdate("linkUrl", e.target.value)} className={selectClass} placeholder="https://..." />
            </div>
          </>
        )}
        ...
        {block.type === EmailBlockType.VIDEO && (
          <>
            <div>
              ...
              <input type="text" value={(block as VideoBlock).url} onChange={(e) => handleUpdate("url", e.target.value)} className={selectClass} placeholder="YouTube or Vimeo URL" />
            </div>
            <div>
              <label className={labelClass}>Thumbnail URL</label>
              <input type="text" value={(block as VideoBlock).thumbnailUrl || ""} onChange={(e) => handleUpdate("thumbnailUrl", e.target.value)} className={selectClass} placeholder="https://..." />
            </div>
          </>
        )}
        ...
        {block.type === EmailBlockType.BUTTON && (
          <>
            <div>
              ...
              <input type="text" value={(block as ButtonBlock).label} onChange={(e) => handleUpdate("label", e.target.value)} className={selectClass} />
            </div>
            <div>
              <label className={labelClass}>URL</label>
              <input type="text" value={(block as ButtonBlock).url} onChange={(e) => handleUpdate("url", e.target.value)} className={selectClass} placeholder="https://..." />
            </div>
          </>
        )}
        ...
        {block.type === EmailBlockType.SPACER && (
          <div>
            ...
            <input type="number" value={(block as SpacerBlock).height || 20} onChange={(e) => handleUpdate("height", parseInt(e.target.value) || 20)} className={selectClass} min={1} max={200} />
          </div>
        )}
        ...
        {block.type === EmailBlockType.HTML && (
          <div>
            ...
            <textarea value={(block as HtmlBlock).content} onChange={(e) => handleUpdate("content", e.target.value)} className={`${textareaClass} font-mono text-xs`} rows={8} />
          </div>
        )}
        ...
        {block.type === EmailBlockType.DISCOUNT_CODE && (
          <>
            <div>
              ...
              <input type="text" value={(block as DiscountCodeBlock).code} onChange={(e) => handleUpdate("code", e.target.value)} className={selectClass} />
            </div>
            <div>
              <label className={labelClass}>Description</label>
              <input type="text" value={(block as DiscountCodeBlock).description || ""} onChange={(e) => handleUpdate("description", e.target.value)} className={selectClass} />
            </div>
          </>
        )}
        ...
        {block.type === EmailBlockType.MENU && (
          <div className="space-y-3">
            ...
            <button
              type="button"
              onClick={() => {
                const currentItems = (block as MenuBlock).items
                const newItems = [
                  ...currentItems,
                  {
        ...
        {block.type === EmailBlockType.DIVIDER && <div className="text-xs text-gray-500 dark:text-gray-400 py-4 text-center">Use the Style tab to customize the divider appearance.</div>}

        {/* Columns - minimal content options */}
        {block.type === EmailBlockType.COLUMNS && <div className="text-xs text-gray-500 dark:text-gray-400 py-4 text-center">Drag blocks into the columns on the canvas to add content.</div>}
      </div>
    </div>
  )
}
