...
import { EmailBlock, EmailBreakpointId, ResponsiveBlockStyle } from "../../../../../types";
...
export const StyleSwitcher: React.FC<StyleSwitcherProps> = ({ block, activeBreakpoint, updateBlock }) => {
    const getCopiedFromBreakpoint = useCallback((): EmailBreakpointId | null => {
        let current = block.style?.[activeBreakpoint];
        const visited = new Set<EmailBreakpointId>([activeBreakpoint]);

        while (typeof current === 'string') {
            const ref = current as EmailBreakpointId;
            if (visited.has(ref)) return null;
            visited.add(ref);
            const next = block.style?.[ref];
    ...
    const copiedFromLabel = copiedFromBreakpoint
        ? (EMAIL_SCREEN_SIZES.find((size) => size.id === copiedFromBreakpoint)?.label ?? copiedFromBreakpoint.toUpperCase())
        : null;

    // Compute which breakpoints currently reference the active breakpoint (or ARE the active)
    const getSelectedFromEmailBlock = useCallback((): Set<EmailBreakpointId> => {
        const selected = new Set<EmailBreakpointId>();
        selected.add(activeBreakpoint);

        EMAIL_BREAKPOINT_IDS.forEach(id => {
            if (id === activeBreakpoint) return;
            const styleVal = block.style?.[id];
    ...
    // Re-sync when block style or activeBreakpoint changes externally
    useEffect(() => {
    ...
    const handleToggleSize = (id: EmailBreakpointId) => {
    ...
    const handleApplyAllToggle = () => {
        if (isApplyAll) {
            ...
            // Select all
            setSelectedSizes(new Set([...EMAIL_BREAKPOINT_IDS]));
        }
        setIsDirty(true);
    };
    ...
    const handleSave = () => {
        ...
        const newStyle: ResponsiveBlockStyle = { ...block.style };
        ...
        EMAIL_BREAKPOINT_IDS.forEach(id => {
            ...
            if (selectedSizes.has(id)) {
                ...
                const currentVal = block.style?.[id];
                if (currentVal === activeBreakpoint) {
                    newStyle[id] = resolveBreakpointStyle(block.style, id);
                }
                // Otherwise leave as-is
            }
        });

        updateBlock(block.id, { style: newStyle });
        setIsDirty(false);
    };
        ...
        <div className="flex flex-col gap-3 bg-gray-50 dark:bg-gray-800/50 p-3 rounded-lg border border-gray-200 dark:border-gray-700">
            ...
            <div className="flex gap-1.5">
                {EMAIL_SCREEN_SIZES.map(size => {
                    const bpId = size.id as EmailBreakpointId;
                    ...
                    return (
                        <button
                            ...
                            className={`flex-1 py-1.5 rounded-md text-xs select-none font-semibold transition-all ${isSelected
