...
export const TypographySection: React.FC<TypographySectionProps> = ({ block: _block, capabilities, getStyleValue, handleStyleUpdate, setActiveSubElement: _setActiveSubElement }) => {
  if (!capabilities.supportsTypography) return null

  const fontWeightValue = String(getStyleValue("fontWeight", "normal"))
  const numericFontWeight = Number(fontWeightValue)
  const isBold = fontWeightValue === "bold" || (!isNaN(numericFontWeight) && numericFontWeight >= 600)
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-1">
          ...
          <select value={String(getStyleValue("fontFamily", "default"))} onChange={(e) => handleStyleUpdate("fontFamily", e.target.value === "default" ? "" : e.target.value)} className="shadow w-full px-2 py-1.5 bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-md text-xs text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary">
        ...
        <div className="grid grid-cols-2 gap-1 w-full">
          ...
          <div className="space-y-1">
            ...
            <select value={String(getStyleValue("fontWeight", "normal"))} onChange={(e) => handleStyleUpdate("fontWeight", e.target.value)} className="shadow w-full px-2 py-1.5 bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-md text-xs text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary">
        ...
        <div className="space-y-1">
          ...
          <div className="grid grid-cols-3 gap-1">
            <button type="button" onClick={() => handleStyleUpdate("fontWeight", isBold ? "normal" : "bold")} className={`px-2 py-1.5 rounded-md border text-xs font-semibold transition-colors ${isBold ? "bg-primary text-white border-primary" : "bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-gray-600"}`}>
              B
            </button>
        ...
        <div className="space-y-1">
          ...
          <input type="text" value={getStyleValue("lineHeight", "1.5")} onChange={(e) => handleStyleUpdate("lineHeight", e.target.value)} className="shadow w-full px-2 py-1.5 bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-md text-xs text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary" placeholder="1.5" />
        </div>
