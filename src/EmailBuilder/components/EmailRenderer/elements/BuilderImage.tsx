...
export default function BuilderImage({ block, isSelected, activeSubElement }: Readonly<BuilderImageProps>) {
  ...
  return (
    <BuilderBlockWrapper block={block} isSelected={isSelected} activeSubElement={activeSubElement}>
      {block.src ? (
        <img src={block.src} alt={block.alt || "Image"} className={`w-full h-auto ${isSelected && activeSubElement === "content" ? "ring-1 ring-primary ring-offset-1" : ""}`} style={{ borderRadius: "inherit", display: "block" }} />
      ) : (
