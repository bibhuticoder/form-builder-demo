...
function ColumnDropZone({
  ...
  return (
    <div ref={setNodeRef} className={`border border-dashed rounded p-2 min-h-[120px] transition-colors ${isOver ? "border-primary bg-primary/5 dark:bg-primary/10" : "border-gray-300 dark:border-gray-600"}`} style={{ width: column.width }}>
        ...
        <div className="w-full space-y-1">
          {column.blocks.map((b) => (
            <BuilderBlockControls key={b.id} block={b} onDelete={deleteBlock} selected={selectedBlockId === b.id} onSelect={onSelectBlock}>
              <BlockRenderer block={b} isSelected={selectedBlockId === b.id} activeSubElement={activeSubElement} selectedBlockId={selectedBlockId} onSelectBlock={onSelectBlock} />
    ...
    <BuilderBlockWrapper block={block} isSelected={isSelected} activeSubElement={activeSubElement}>
      <div className="flex gap-2">
        {block.columns.map((col) => (
          <ColumnDropZone key={col.id} columnsBlockId={block.id} column={col} selectedBlockId={selectedBlockId} onSelectBlock={onSelectBlock} activeSubElement={activeSubElement} />
        ))}
      </div>
    </BuilderBlockWrapper>
  )
