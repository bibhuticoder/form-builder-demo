...
export const EmailBuilderProvider: React.FC<EmailBuilderProviderProps> = ({ initialContent, children }) => {
  const { state: jsonContent, set: setJsonContentState, undo, redo, canUndo, canRedo, pointer } = useHistory<EmailTemplate>(initialContent)
  ...
  const addBlock = useCallback(
    (block: EmailBlock, position?: string | number) => {
      setJsonContentState((old) => {
  ...
  const updateBlockStyleBatch = useCallback(
    (blockId: string, styleUpdates: Record<string, unknown>) => {
      setJsonContentState((old) => ({
        ...old,
  ...
  const value = useMemo(
    ...
    [jsonContent, activeSubElement, canvasWidth, activeBreakpoint, setJsonContent, setActiveSubElement, setCanvasWidth, updateTemplateName, updateTemplateSettings, addBlock, updateBlock, updateBlockStyleBatch, deleteBlock, reorderBlocks, undo, redo, canUndo, canRedo, pointer, activeView],
  )
