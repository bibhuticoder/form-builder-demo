...
import { Button, IconInput, Card } from "../../../../components";
...
const DraggableElementCard: React.FC<{ item: ElementItem }> = ({ item }) => {
  ...
  return (
    <div ref={setNodeRef} {...attributes} {...listeners} style={{ opacity: isDragging ? 0.5 : 1 }}>
      <Card className="p-2 border border-gray-300 dark:border-gray-600 hover:border-blue-500 dark:hover:border-blue-400 transition-colors"  aria-label={item.label}>
        <div className="flex flex-col items-center justify-center gap-2 p-3 group cursor-grab select-none">
          <item.icon className="w-4 h-4 text-gray-600 dark:text-gray-400 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors" />
          <span className="text-xs font-medium text-gray-700 dark:text-gray-300 group-hover:text-blue-600 dark:group-hover:text-blue-400 text-center transition-colors">{item.label}</span>
        </div>
      </Card>
    </div>
  );
          ...
          <>
            <div className="p-3 border-b border-gray-200 dark:border-gray-700">
              <IconInput
                ...
                inputStyles="!text-sm !text-gray-900 dark:!text-white placeholder:!text-gray-400 dark:placeholder:!text-gray-500"
              />
            </div>
            ...
            {ELEMENT_GROUPS.map((group, idx) => (
              <div key={idx} className="w-full flex flex-col items-center gap-2">
                <div className="w-full flex justify-center py-4 border-b border-gray-200 dark:border-gray-700">
                  <span
                    className="text-[9px] font-medium text-gray-500 dark:text-gray-400 tracking-wider uppercase"
